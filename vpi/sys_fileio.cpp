...
export module sys.fileio;
import sys.priv;

export {
    PLI_INT32 sys_fopen_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fopen_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fopenrwa_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fclose_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fflush_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_common_fd_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fgets_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fgets_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fread_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fread_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fseek_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fseek_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_ferror_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_ferror_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fputc_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_ungetc_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
}
...
PLI_INT32 sys_fread_calltf(ICARUS_VPI_CONST PLI_BYTE8* name)
{
    ...
    assert(width > 0);
    words = (width - 1) / 32 + 1;
    vector = (s_vpi_vecval *)calloc(words, sizeof(s_vpi_vecval));
    bpe = (width + 7) / 8;
