...
static void initialize_control_fields_def(p_table_mod table)
{
    ...
    assert(val);
    table->control.info.interp = (char*)memset(val, IVL_LINEAR_INTERP, width);

    /* Set the default low extrapolation for each dimension. */
    val = (char*)malloc(width);
    assert(val);
    table->control.info.extrap_low = (char*)memset(val, IVL_LINEAR_EXTRAP, width);

    /* Set the default low extrapolation for each dimension. */
    val = (char*)malloc(width);
    assert(val);
    table->control.info.extrap_high = (char*)memset(val, IVL_LINEAR_EXTRAP, width);
...
static PLI_INT32 sys_table_model_calltf(ICARUS_VPI_CONST PLI_BYTE8* name)
{
    ...
    table = (p_table_mod)vpi_get_userdata(callh);
...
void table_model_register(void)
{
    ...
    tf_data.tfname = const_cast<char*>("$table_model");
    ...
    tf_data.user_data = const_cast<char*>("$table_model");
