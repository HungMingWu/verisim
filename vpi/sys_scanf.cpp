...
export module sys.scanf;
import sys.priv;

export {
	PLI_INT32 sys_fscanf_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
	PLI_INT32 sys_fscanf_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
	PLI_INT32 sys_sscanf_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
	PLI_INT32 sys_sscanf_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
}
	...
	PLI_INT32 type)
{
	...
	while (strchr(match, ch) && (len < width)) {
		...
		strval = (char *)realloc(strval, len + 2);
		strval[len++] = ch;
                                 ...
                                 unsigned suppress_flag, ICARUS_VPI_CONST PLI_BYTE8 *name)
{
	...
	/* Mask the upper bits to match the specified width when required. */
      if (width != UINT_MAX) {
	    PLI_INT32 mask = UINT_MAX >> (32U - ((width - 1U) % 32U + 1U));
	...
	ICARUS_VPI_CONST PLI_BYTE8* name)
{
	...
	mask = std::numeric_limits<uint32_t>::max() >> (32U - ((len - 1U) % 32U + 1U));
