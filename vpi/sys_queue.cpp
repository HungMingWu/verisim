...
export {
     ...
     */
    PLI_INT32 sys_q_initialize_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * Check that the given $q_initialize() call has valid arguments.
     */
    PLI_INT32 sys_q_initialize_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * The runtime code for $q_add().
     */
    PLI_INT32 sys_q_add_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * Check that the given $q_add() call has valid arguments.
     */
    PLI_INT32 sys_q_add_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * The runtime code for $q_remove().
     */
    PLI_INT32 sys_q_remove_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * Check that the given $q_remove() call has valid arguments.
     */
    PLI_INT32 sys_q_remove_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * The runtime code for $q_full().
     */
    PLI_INT32 sys_q_full_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * Check that the given $q_full() call has valid arguments.
     */
    PLI_INT32 sys_q_full_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * The runtime code for $q_exam().
     */
    PLI_INT32 sys_q_exam_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * Check that the given $q_exam() call has valid arguments.
     */
    PLI_INT32 sys_q_exam_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /*
     * This routine is called at the end of simulation to free the queue memory.
     */
    PLI_INT32 cleanup_queue(p_cb_data cause);
}
