...
export module sys.readmem;
import sys.priv;
import sys.readmem.lex;

export {
    PLI_INT32 sys_readmem_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_mem_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_readmempath_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_writemem_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 free_readmempath(p_cb_data cb_data);
}
...
PLI_INT32 sys_readmem_calltf(ICARUS_VPI_CONST PLI_BYTE8* name)
{
   ...
   back to this code */
    ...
    value.value.vector = (t_vpi_vecval * )calloc((wwid + 31) / 32, sizeof(s_vpi_vecval));
