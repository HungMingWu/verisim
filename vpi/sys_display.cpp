...
export module sys.display;
import sys.priv;

export {
    void check_command_line_args();

    /* This implements the $sformatf, $display/$fdisplay
     * and the $write/$fwrite based tasks. */
    PLI_INT32 sys_display_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /* Check the $display, $write, $fdisplay and $fwrite based tasks. */
    PLI_INT32 sys_display_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /* This implements both the $strobe and $fstrobe based tasks. */
    PLI_INT32 sys_strobe_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    /* Check both the $strobe and $fstrobe based tasks. */
    PLI_INT32 sys_strobe_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_monitor_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_monitor_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_monitoron_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_monitoroff_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_swrite_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_swrite_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_sformat_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_sformat_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_timeformat_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_timeformat_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_printtimescale_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_printtimescale_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);

    PLI_INT32 sys_severity_calltf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_sformatf_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_fatal_compiletf(ICARUS_VPI_CONST PLI_BYTE8* name);
    PLI_INT32 sys_end_of_compile(p_cb_data cb_data);
    PLI_INT32 sys_end_of_simulation(p_cb_data cb_data);
}
...
static void array_from_iterator(struct strobe_cb_info* info, vpiHandle argv)
{
    if (argv) {
        vpiHandle item;
        unsigned nitems = 1;
        vpiHandle* items = (vpiHandle*)malloc(sizeof(vpiHandle));
        ...
        for (item = vpi_scan(argv); item; item = vpi_scan(argv)) {
            items = (vpiHandle*)realloc(items, (nitems + 1) * sizeof(vpiHandle));
            items[nitems] = item;
            nitems += 1;
        }
    ...
    unsigned int* idx)
{
    ...
    result = (char *)malloc(ini_size * sizeof(char));
            ...
            if (value.format == vpiSuppressVal) {
                ...
                if (ld_zero == 1) {
                    ...
                    /* Pad with leading zeros. */
                        ...
                        cp = (char *)malloc((width + 1) * sizeof(char));
                        memset(cp, '0', pad);
                ...
                if (size > ini_size) result = (char*)realloc(result, size * sizeof(char));
        ...
        if (*idx >= info->nitems) {
            ...
            if (value.format == vpiSuppressVal) {
                ...
                if (ljust == 0 && ld_zero == 1 && (signed)swidth < width) {
                    tbuf = (char*)malloc((width + 1) * sizeof(char));
                    pad = (unsigned)width - swidth;
                }
                else {
                    tbuf = (char*)malloc((swidth + 1) * sizeof(char));
                }
                cpb = tbuf;
        ...
        *idx += 1;
        if (*idx >= info->nitems) {
            ...
            if (value.format == vpiSuppressVal) {
                ...
                tbuf = (char*)malloc((513 + suff_len) * sizeof(char));
    ...
    const struct strobe_cb_info* info, unsigned int* idx)
{
    ...
    *rtn = strdup("");
    ...
    while (*cp) {
        ...
        if (cnt > 0) {
            *rtn = (char*)realloc(*rtn, (size + cnt) * sizeof(char));
...
static char* get_display(unsigned int* rtnsz, const struct strobe_cb_info* info)
{
    ...
    for (idx = 0; idx < info->nitems; idx += 1) {
        ...
        case vpiParameter:
            ...
            rtn = (char*)realloc(rtn, (size + width) * sizeof(char));
...
PLI_INT32 sys_strobe_calltf(ICARUS_VPI_CONST PLI_BYTE8* name)
{
    ...
    info = (struct strobe_cb_info* )calloc(1, sizeof(struct strobe_cb_info));
    info->fd_mcd = fd_mcd;
    /* We could use vpi_get_str(vpiName, callh) to get the task name,
     * but name is already defined. */
    info->name = name;
...
PLI_INT32 sys_monitor_calltf(ICARUS_VPI_CONST PLI_BYTE8* name)
{
    ...
    monitor_callbacks = (vpiHandle *)calloc(monitor_info.nitems, sizeof(vpiHandle));
