...
static PLI_INT32 simparam_str_calltf(ICARUS_VPI_CONST PLI_BYTE8* name_ext)
{
    ...
    if (strcmp(param, "analysis_name") == 0) {
        ...
        snprintf(path, MAX_STRING_RESULT, path1.string().c_str());
        retval = strdup(path);
    }
...
void vams_simparam_register(void)
{
    ...
    tf_data.sizetf = 0;
    tf_data.tfname = const_cast<char*>("$simparam");
    tf_data.user_data = const_cast<char*>("");
    ...
    tf_data.tfname = const_cast<char*>("$simparam$str");
    tf_data.user_data = const_cast<char*>("$str");
    res = vpi_register_systf(&tf_data);
    vpip_make_systf_system_defined(res);
}
