...
public:
     ...
     */
    ///@{
    dylib(const char *dir_path, const char *lib_name, bool global_symbol, bool decorations = add_filename_decorations) {
        ...
        m_handle = open((final_path + final_name).c_str(), global_symbol);

        if (!m_handle)
            throw load_error("Could not load library \"" + final_path + final_name + "\"\n" + get_error_description());
    }

    dylib(const std::string &dir_path, const std::string &lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib(dir_path.c_str(), lib_name.c_str(), global_symbol, decorations) {}

    dylib(const std::string &dir_path, const char *lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib(dir_path.c_str(), lib_name, global_symbol, decorations) {}

    dylib(const char *dir_path, const std::string &lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib(dir_path, lib_name.c_str(), global_symbol, decorations) {}

    explicit dylib(const std::string &lib_name, bool decorations = add_filename_decorations)
        : dylib("", lib_name.c_str(), decorations) {}

    explicit dylib(const char *lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib("", lib_name, global_symbol, decorations) {}

#ifdef DYLIB_CPP17
    explicit dylib(const std::filesystem::path &lib_path, bool global_symbol = false)
        : dylib("", lib_path.string().c_str(), global_symbol, no_filename_decorations) {}

    dylib(const std::filesystem::path &dir_path, const std::string &lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib(dir_path.string().c_str(), lib_name.c_str(), global_symbol, decorations) {}

    dylib(const std::filesystem::path &dir_path, const char *lib_name, bool global_symbol = false, bool decorations = add_filename_decorations)
        : dylib(dir_path.string().c_str(), lib_name, global_symbol, decorations) {}
#endif
    ///@}
...
protected:
    ...
    static native_handle_type open(const char *path, bool global_flag) noexcept {
#if defined(_WIN32) || defined(_WIN64)
        return LoadLibraryA(path);
#else
        return dlopen(path, RTLD_LAZY | (global_flag ? RTLD_GLOBAL : RTLD_LOCAL));
#endif
    }
